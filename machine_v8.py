...
(modules/processor_v8.source.json — latched port A, memory-mapped port B,
...
from assembler import Program, assemble_program, encode
...
_CHUNK1_WIDTH = total_addresses()  # 2480: also v10's per-chunk width (modules/v10_addr_map.fnet)
...
def _name(addr):
    if addr > _CHUNK1_WIDTH:
        ...
        # offset by _CHUNK1_WIDTH per register (modules/v10_addr_map.fnet
...
class MachineConfig:
    ...
    the design file is the source of truth (NETLIST_PLAN 'config combinators
    are fnet-owned')."""
    pc_addr: int
    mmap_b: int
...
class IR8:
    """v8 program builder. Stage semantics as in scheduler.py: within a stage
