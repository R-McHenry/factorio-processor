...
machine_v8 now knows the vector zone's timing (`MachineConfig.vec_reach`), so
...
from machine_v8 import (IR8, WRITE_TO_CELL, check_pulse_widths,  # noqa: E402
...
from vlang import (A_MAX, A_MIN, A_MUL, A_SUB, B_MUL, B_SQ,  # noqa: E402,F401
...
def vector_roundtrip(cfg, rows, lane_count):
    ...
    description = (
        ...
        "machine_v8 scheduled all of it with no vector awareness at all."
    )
...
def mandelbrot(cfg, rows, lane_count):
    ...
    pulse_imm to a memory row, and machine_v8 schedules the lot.
...
def mandelbrot_dsl(cfg, rows, lane_count):
    """`tools/build_v10_tests.py:mandelbrot`, recompiled from vlang.
