#!/usr/bin/env python3
"""Self-tests for fnet/hdl.py (plain asserts, run with the venv python).

Covers ../plan/NETLIST_PLAN.md migration step 1: parsing, template substitution,
...
must be entity- and connectivity-equivalent to demo/demo_circuit.source.json.
"""
import json
import sys
import tempfile
...
sys.path.insert(0, str(MAIN))          # main/ on the path
ROOT = MAIN

from fnet.hdl import (  # noqa: E402
...
def test_demo_matches_demo_circuit_topology():
    design = build(MAIN / "fnet" / "demo" / "netlist_demo.fnet", None, None, None, None,
                   check_only=True)
    wires = design.wires()
    payload = design.emit_source("netlist_demo", wires)
    reference = json.loads(
        (MAIN / "fnet" / "demo" / "demo_circuit.source.json").read_text(encoding="utf-8")
    )
...
""")
    ...
    from bench.processor_tb import signal_table_rows
...
def test_mem_refs_resolve_through_design_space():
    from bench.processor_tb import design_address_space, resolve_signal_ref
    design = build(MAIN / "fnet" / "demo" / "netlist_demo.fnet", None, None, None, None,
                   check_only=True)
    payload = design.emit_source("netlist_demo", design.wires())
...
""")
    ...
    # the signals map carries the flag processor.isa will read at schedule time
