...
`plan/ROADMAP.md` §1 and §5 carry it: `jump_rel_if` and forward labels (which
is why there is one loop per program and no `if`), the scalar `int` type,
subscripting, predication, and the text front-end. The deliberately-deferred
items — the vector decoder, delay-slot filling, and the fact that a tick
simulator is explicitly not wanted — are §3, §6 and §6 respectively.
