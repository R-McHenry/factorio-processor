...
- ROM constant cleared (program variants inject their own ROM via
  the archived build_closed_loop.py; the open-loop benches need an inert machine)
...
def generated_config() -> dict:
    ...
    replaced with the canonical values derived from processor.isa/signal_space —
    the same philosophy as the address tables' signal_table expansion."""
    from processor.isa import ACCUMULATE_SIGNALS, MMAP_B_SIGNAL
    return {
