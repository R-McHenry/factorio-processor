#!/usr/bin/env python3
"""vlang — the language for the v10 vector processor (docs/language.md).
...
The fifth layer, and the only new code between the surface syntax and IR8:
    ...
    vlang  ------------->   IR8 --schedule8-->   ROM      <-- this file
                            bench/processor_tb.py         verification
...
never a vector control row — so the planned instruction decoder (docs/language.md
§7) is a backend swap that no compiled program can notice.

WHAT THE MACHINE IS, FROM HERE (docs/language.md §1). Not a register machine:
...
THREE HARDWARE FACTS THAT SHAPE THE OUTPUT (docs/mandelbrot.md §4):
...
The surface is a Python DSL (docs/language.md §2, decided): expression trees come
...
def _check_imm(k, what):
    if not (IMM_MIN <= k <= IMM_MAX):
        raise CompileError(
            ...
            "not precision appetite (docs/mandelbrot.md §4)")
    return k


# ---------------------------------------------------------------------------
# 1. The DAG (docs/language.md §4a)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. Instruction selection (docs/language.md §4b)
# ---------------------------------------------------------------------------
...
class Insn:
    ...
    def triple(self):
        """The abstract `(unit, operands, dest)` triple of docs/language.md §5 step
...
# ---------------------------------------------------------------------------
# 3. Register allocation (docs/language.md §4c)
# ---------------------------------------------------------------------------
...
class Machine:
    ...
    of the body. That keeps the surface exactly as docs/language.md §2 sketched it,
    ...
    def _bin(self, op, a, b):
        ...
        if op == "sub" and bn.is_const:
            ...
            # that dropped the lane (docs/mandelbrot.md §4).
    ...
    # -- control flow -------------------------------------------------------
    def loop(self, count):
        """`with m.loop(n):` — the counted loop of docs/language.md §4e.
        ...
        SHADOW low memory and port A reads the memory bus (docs/mandelbrot.md §4).
        """
        if self._loops:
            raise CompileError("v1 compiles exactly one loop per program "
                               "(docs/language.md §5 step 7)")
    ...
    def _assign_dests(self, blk, uses):
        """THE PEEPHOLE THAT PAYS (docs/language.md §4c): a value whose only use is
        ...
        for st in sorted(seen, key=lambda s: (s.first, s.last, s.name)):
            ...
            if not free:
                ...
                raise CompileError(
                    ...
                    "hard error rather than a silent catastrophe (docs/language.md §4c)")
            st.reg = free[0]
            active.append(st)
    ...
    def emit(self, ir=None, halt=True, label="vloop"):
        ...
        for blk in self.blocks:
            ...
            if blk.kind == "loop":
                ...
                # reads the memory bus (docs/mandelbrot.md §4)
