...
- **Reductions beyond `vred_sum`** — count, max, min, argmax, argmin, the CMP
  flags. Addresses allocated, hardware not. `vred_count` (popcount) would have
  read out "how many lanes are still inside" directly instead of costing a
  sticky mask and a whole-frame sum; worth doing before the next kernel.
- **Multi-chunk port A** — reading a vector lane by a chunk-2..9 address. Not
  needed: `vec_rlane_addr` already reads any lane of the bus, one at a time.
  Reading a whole 64-lane row IS 64 reads, which is what makes the scanline
  the expensive third of this program.
- **The quality-transfer unary op** (`vqual_level`).
- **A second PC / instruction decoder** feeding the interconnect directly
  instead of memory-mapped config — the designer's stated future direction.
  Nothing built is wasted; a decoder becomes another driver of the same
  select/erase lines.
- **A conditional relative jump's compiler half** — the ISA half is measured
  (V8.md TODO #1); what is missing is `jump_rel_if` + forward labels in IR8,
  which is also what caps vlang at one loop and no `if`. The mandelbrot loop pays
  the write-path absolute jump every pass.
